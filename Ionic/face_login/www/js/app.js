...
facebookExample.controller("LoginController", function($scope, $cordovaOauth, $localStorage, $location) {
    ...
    $scope.login = function() {
            $cordovaOauth.facebook("1619286494969231", ["public_profile","email", "user_website"]).then(function(result) {
...
facebookExample.controller("ProfileController", function($scope, $http, $localStorage, $location) {
    ...
    $scope.init = function() {
        if($localStorage.hasOwnProperty("accessToken") === true) {
            alert("entro");
            $http.post("https://graph.facebook.com/v2.2/me", { params: { access_token: $localStorage.accessToken, fields: "id,name,gender,locale,website", format: "json" }}).then(function(result) {
